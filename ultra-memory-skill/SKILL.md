...
## What to Persist (from Rigor Pack memory-hygiene benchmarks)

Benchmark-tested rules for what enters persistent memory.

### One Fact Per Entry, Dated
Every persistent note gets a date and says ONE thing. "As of 2026-07-02" ages honestly. Undated memory is unreliable.
...
What was considered and WHY it was rejected matters more than what was chosen. "Chose X over Y because Z" survives context loss.
...
## Knowledge Graph Layer (from Graphify-Labs/graphify)

Graph traversal complements keyword/vector search. While SSC Router does keyword matching and atoms do fact lookup, the knowledge graph maps *relationships* between concepts.

### EXTRACTED vs INFERRED Edges

Every connection in the graph is tagged:

| Tag | Meaning | Example |
|-----|---------|---------|
| **EXTRACTED** | Explicit in the source file | `module A` imports `module B` |
| **INFERRED** | Resolved by graphify's resolver | `function X` calls `function Y` (via AST analysis) |

**Applied to memory atoms:** Replace `confidence: alto/médio/baixo` with `origin: EXTRACTED | INFERRED`:

```markdown
- PM2 é o único dono do gateway. Nunca criar Windows Scheduled Tasks.
  - Source: s006-ssc-skill-incident.md
  - Date: 2026-06-27
  - Origin: EXTRACTED (incidente real, regra documentada)

- Graphify resolveu que módulo A depende de módulo B
  - Source: graphify-out/graph.json
  - Date: 2026-07-06
  - Origin: INFERRED (AST analysis)
```
...
|--------|----------|------|
| SSC Router (keyword) | Broad topic search | `ssc-router.ps1 -Query "topic"` |
| Drill-down (hierarchy) | Specific fact lookup | `node scripts/drill-down.js "query"` |
| Knowledge graph (traversal) | Relationship mapping | `graphify path "A" "B"` |
| Graph query (BFS) | Question answering | `graphify query "what depends on X?"` |
...
graphify path "FastAPI" "Database"

# Explain a concept and its neighbors
graphify explain "SSC Router"

# Update graph after code changes (no LLM cost)
graphify update .
...
### Integration with SSC v2

The knowledge graph adds a fifth retrieval layer:
...
Graph is queried AFTER atoms if the question is about relationships, not facts.
...
python -m graphify claw install

# Build graph (needs LLM API key for docs, not for code)
python -m graphify extract . --backend gemini

# Verify
python -m graphify query "SSC Router"
```

**Zero vendor lock-in:** graphify is MIT license, runs locally, code parsing is tree-sitter (no API). Only docs/media extraction uses LLM.
