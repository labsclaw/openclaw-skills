...
description: >-
  ...
  fallback failures, or comparing openclaw.json config against actual API
  responses. Also use when users say "check models", "list free models",
  "which models are available", "are my fallbacks working", "audit models",
  "model health check", "free model monitor", or want to verify provider
  model status. Trigger terms: free models, model audit, model check,
  fallback broken, model status, provider models, model monitor, compare
  config, model health, list providers.
---
...
# Or workspace-local
cp -r ultra-models-skill/ <workspace>/ultra-models-skill/
```
...
```powershell
# Verify keys exist
powershell -Command "Get-Content '$env:USERPROFILE\.openclaw\.env' | Select-String 'API_KEY'"
```
...
Returns model IDs, names, and counts per provider.
...
Cross-references `openclaw.json` against live API responses. Reports:
...
├── scripts/
│   ├── list-free-models.ps1    ← Queries all 4 provider APIs
│   ├── compare-config.ps1      ← API vs config cross-reference (main tool)
